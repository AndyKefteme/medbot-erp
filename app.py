...
# Дозволяємо HTTP для OAuth (DisCloud зазвичай використовує проксі)
...
# --- БАЗА ДАНИХ ---
# Файл бази буде зберігатися в корені додатка на DisCloud
...
@st.cache_resource
def load_ocr():
    # На DisCloud може бути мало RAM, тому gpu=False обов'язково
...
def compress_image(image_file):
    """Стискає зображення, щоб уникнути помилки 413 на DisCloud/Discord."""
...
if __name__ == "__main__":
    # DisCloud автоматично надає PORT, якщо ні — використовуємо стандартний 8501
    port = int(os.environ.get("PORT", 8501))
    # Ця частина коду не запускається всередині Streamlit, 
    # вона потрібна лише як точка входу для DisCloud