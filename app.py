...
# --- НАЛАШТУВАННЯ ---
# Використовуємо st.secrets для безпеки на Streamlit Cloud
try:
    config = st.secrets["discord"]
except Exception:
    st.error("Помилка: Налаштування 'secrets' не знайдено!")
    st.stop()
...
if menu == "🚪 Вихід":
    ...
    if not all(current_coords.values()):
        st.warning("⚠️ Налаштуйте координати в розділі Налаштування!")
    else:
