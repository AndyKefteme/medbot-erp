...
# Виправляємо опечатку: використовуємо єдину назву MODEL_DIR
...
if "discord" not in st.secrets:
    st.error("Критична помилка: Налаштуйте [discord] секцію в Secrets на Streamlit Cloud!")
    st.stop()
...
def load_reader():
    with st.spinner("🏥 Активація штучного інтелекту..."):
        # Використовуємо нашу папку MODEL_DIR
...
def login_page():
    ...
    with col:
        st.markdown(f'''
            <div style="text-align: center;">
                <p>Необхідно авторизуватися через Discord</p>
    ...
    if "code" in params:
        try:
            ...
            if not is_allowed:
                st.error("🚫 Відсутня необхідна роль.")
                return
...
if menu == "🚪 Вихід":
    ...
    if not all(current_c.values()):
        ...
        files = st.file_uploader("Фото документів", accept_multiple_files=True, type=['jpg', 'png'])
        ...
        if 'scan_results' in st.session_state:
            ...
            for i, item in enumerate(st.session_state.scan_results):
                ...
                s = c1.text_input(f"Прізвище #{i}", item['Surname'], key=f"s{i}")
                n = c2.text_input(f"Ім'я #{i}", item['Name'], key=f"n{i}")
                u = c3.text_input(f"ID #{i}", item['ID'], key=f"u{i}")
            ...
            if st.button("🚀 ВІДПРАВИТИ ЗВІТ"):
                ...
                cursor.execute("INSERT INTO logs VALUES (?, ?, ?, ?)", (user['id'], user['username'], len(final), datetime.now().strftime("%H:%M")))
                conn.commit()
